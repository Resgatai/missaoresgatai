...
export default async function PublicHome({ searchParams }: { searchParams: Promise<{ inscrito?: string; erro?: string }> }) {
  ...
  return <main className="min-h-screen bg-[#fffdf6] text-[#2d2926]">
    ...
    <section id="conteudos" className="border-y border-[#eadfce] bg-white px-5 py-14">
      <div className="mx-auto max-w-6xl"><div className="flex items-center gap-3"><PlayCircle className="text-[#7b4b2a]"/><div><p className="text-xs font-bold uppercase tracking-widest text-[#7b4b2a]">Edificacao</p><h2 className="mt-1 text-3xl font-bold">Conteudos recentes</h2></div></div><div className="mt-7 grid gap-4 md:grid-cols-3">{contents.map((content) => <a key={content.id} href={content.url} target="_blank" rel="noreferrer" className="rounded-xl border border-[#eadfce] p-5 transition hover:border-[#7b4b2a]"><p className="text-xs font-bold uppercase text-[#7b4b2a]">{content.type}</p><h3 className="mt-2 font-bold">{content.title}</h3>{content.description && <p className="mt-2 text-sm leading-6 text-stone-500">{content.description}</p>}</a>)}</div>{radio && <div className="mt-8 rounded-xl bg-[#2d2926] p-5 text-white"><div className="flex items-center gap-3"><Radio className="text-[#e9d33b]"/><div><h3 className="font-bold">{radio.name}</h3><p className="text-sm text-stone-300">Ouca nossa radio.</p></div></div><audio controls preload="none" className="mt-4 w-full"><source src={radio.streamUrl}/></audio></div>}</div>
    </section>
